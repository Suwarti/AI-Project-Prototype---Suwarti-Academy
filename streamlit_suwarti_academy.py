...
# - Jawaban harus detil, mengikuti struktur asli PDF.
# - Menggabungkan semua konteks (chunk) yang diambil.
# - CLEANSING via fungsi; aman UTF-8; tanpa daftar PDF/Context; tanpa pesan indexing selesai.
# ------------------------------------------------------------
...
with st.sidebar:
    ...
    persist_path = st.text_input("💾 Folder Chroma", value="suwarti_chroma")
...
if rebuild:
    if not api_key:
        ...
        try:
            ...
            if not pdf_paths:
                st.warning(ensure_utf8(f"Tidak ditemukan file PDF di folder: {pdf_folder}"))
            ...
            for path in pdf_paths:
                try:
                    ...
                    if not text.strip():
                        st.warning(ensure_utf8(f"{os.path.basename(path)}: tidak ada teks terdeteksi."))
                        continue
...
if user_q:
    ...
    if not api_key:
        answer = "Masukkan GEMINI API Key di sidebar atau via ENV (GEMINI_API_KEY)."
    elif not GENAI_OK:
